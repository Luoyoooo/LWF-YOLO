...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            stride_device = torch.device("cuda:0") if torch.cuda.is_available() else orig_device
            if stride_device != orig_device:
                self.model.to(stride_device)
            with torch.no_grad():
                stride = torch.tensor(
                    [s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s, device=stride_device))]
                )
            m.stride = stride.to(orig_device)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in repeat_modules:
                ...
                n = 1
            if m in {C3k2, DCGFormer}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m is A2C2f:
            ...
            c2 = ch[f[-1]]
        elif m is EdgeFusion:
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [[ch[x] for x in f], c2]
        elif m is MSEFE:
            if not isinstance(f, list) or len(f) != 2:
                raise ValueError(f"MSEFE expects two input indices [edge_source, backbone_source], but got {f}.")
            c1 = ch[f[0]]
            c2 = ch[f[1]]
            args = [c1, *args]
            if len(args) == 1:
