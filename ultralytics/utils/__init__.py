...
class JSONDict(dict):
    ...
    def _save(self):
        ...
        try:
            save_dir = self.file_path.parent
            probe_dir = save_dir
            while not probe_dir.exists() and probe_dir != probe_dir.parent:
                probe_dir = probe_dir.parent
            if not is_dir_writeable(probe_dir):
                return
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        with torch_distributed_zero_first(LOCAL_RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                if self._is_file_writeable():
                    LOGGER.info(f"Creating new Ultralytics Settings v{version} file ✅ {self.help_msg}")
                    self.reset()
                else:
                    super().update(self.defaults)

            self._validate_settings()

    def _is_file_writeable(self):
        """Check whether the settings file or its nearest existing parent directory is writable."""
        probe_dir = self.file.parent
        while not probe_dir.exists() and probe_dir != probe_dir.parent:
            probe_dir = probe_dir.parent
        return is_dir_writeable(probe_dir)
