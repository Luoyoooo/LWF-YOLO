...
from .metaformer import LayerNormGeneral, DCGFormerBlock
...
def _make_divisible(v, divisor, min_value=None):
    """Make channel divisible for DyReLU."""
...
class HSigmoid(nn.Module):
    def __init__(self, bias=3.0, divisor=6.0, inplace=True):
        super().__init__()
        self.bias = bias
    ...
    def forward(self, x):
        return F.relu6(x + self.bias, inplace=self.inplace) / self.divisor


def build_activation_layer(cfg):
    """Minimal activation builder for DyDCN."""
    if cfg is None:
        return nn.Identity()
    act_type = cfg.get("type", "")
    if act_type in {"ReLU", "relu"}:
        return nn.ReLU(inplace=True)
    if act_type in {"Sigmoid", "sigmoid"}:
        return nn.Sigmoid()
    if act_type in {"HSigmoid", "h_sigmoid"}:
...
class DyReLU(nn.Module):
    def __init__(
        ...
        self.exp = 4 if k2 and use_bias else 2 if use_bias else 2 if k2 else 1
        self.init_a = init_a
        self.init_b = init_b
        ...
        self.spa = nn.Sequential(nn.Conv2d(inp, 1, 1), nn.BatchNorm2d(1)) if use_spatial else None
    ...
    def forward(self, x):
        if x.dim() == 4:
            ...
            if self.spa is not None:
                out = out * torch.sigmoid(self.spa(x))
            return out
        return x


class DyDCNBlock(nn.Module):
    """DyDCN block using DCNv3."""
